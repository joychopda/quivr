...
def _mcp_request(server_url: str, payload: Dict[str, Any], token: str | None = None) -> Dict[str, Any]:
    ...
    resp = _gr_client.enforce(_gr_site, resp, content_type='application/json', variable_name='resp', source_file=__file__, before_line=25)
    return resp.json()
...
def connect_to_mcp_server(server_url: str) -> ToolWrapper:
    ...
    # LINEAJE: enforce() `_lineaje_payload` at agent->log log_emit — scan flagged AI_APP_SEC_023 (Client must validate and sanitize any output from a MCP server); AI_APP_SEC_029 (Agent must validate, sanitize LLM output including for presence of eval or any dynamic code execution primitive in LLM output.); AI_APP_SEC_032 (Do not allow malicious content via hidden prompts written in leetspeak.). Mask/block; do not remove without review. site_id='site:sha256:9bb36be702d979861f7e298da04a213abfb6ccff60dcd2b929951c1cd76f3fa3'
    _gr_client = _lineaje_load_gr_client()
