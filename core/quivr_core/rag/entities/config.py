...
def normalize_to_env_variable_name(name: str) -> str:
    ...
    # LINEAJE: enforce() `env_variable_name` at agent->user_interface data_egress — scan flagged AI_APP_SEC_006 (Use only LLMs from the organization's approved list.); AI_IAC_023 (Chatbot and AI interfaces must disclose AI identity to the user); AI_IAC_031 (AI model endpoints must enforce role-based access control with minimal OAuth scopes). Mask/block; do not remove without review. site_id='site:sha256:7cca50e88e4212ee7155cedb4f72038e396106f56efe60768505a125a3475e60'
...
class LLMEndpointConfig(QuivrBaseConfig):
    ...
    def set_api_key(self, force_reset: bool = False):
        ...
        if not self.llm_api_key:
            ...
            # LINEAJE: enforce() `_lineaje_payload` at agent->log log_emit — scan flagged AI_APP_SEC_006 (Use only LLMs from the organization's approved list.); AI_IAC_023 (Chatbot and AI interfaces must disclose AI identity to the user); AI_IAC_SEC_020 (Restrict AI agents to an explicit tool allow list.). Mask/block; do not remove without review. site_id='site:sha256:6b93fde637d531cb05b24ec1a399edcda9d00da44d82d2e8278fff44e82bb838'
            _gr_client = _lineaje_load_gr_client()
