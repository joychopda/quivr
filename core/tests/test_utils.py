...
def test_model_supports_function_calling():
    ...
    # LINEAJE: enforce() `_lineaje_payload` at html->user_interface data_egress — scan flagged AI_APP_SEC_006 (Use only LLMs from the organization's approved list.). Mask/block; do not remove without review. site_id='site:sha256:978e03c4c476ecadc72d8848ea3dc4812400dceeef5760ee36885a2bd3f70829'
    _gr_client = _lineaje_load_gr_client()
    _gr_site = _gr_client.SiteDescriptor(site_id='site:sha256:978e03c4c476ecadc72d8848ea3dc4812400dceeef5760ee36885a2bd3f70829', phase='data_egress', boundary={'source': 'html', 'sink': 'user_interface'}, candidate_policies=[{'policy_id': 'AI_DAT_SEC_012', 'guardrail_id': 'Mask PII on UI', 'policy_version': '2026.08.1'}], fail_mode='BLOCK', source_type='html', destination_type='user_interface')
...
def test_parse_chunk_response_nofunc_calling():
    ...
    for i in range(10):
        # LINEAJE: enforce() `rolling_msg` at agent->tool post_tool — scan flagged AI_APP_SEC_014 (MCP server must validate and sanitize all input). Mask/block; do not remove without review. site_id='site:sha256:f6d6073e12cd8740c5bf06600b2ad2332bfade00335b979c3a024cc5e64ffd20'
        _gr_client = _lineaje_load_gr_client()
        _gr_site = _gr_client.SiteDescriptor(site_id='site:sha256:f6d6073e12cd8740c5bf06600b2ad2332bfade00335b979c3a024cc5e64ffd20', phase='post_tool', boundary={'source': 'tool_result', 'sink': 'agent_message'}, candidate_policies=[], fail_mode='ALLOW_WITH_AUDIT', source_type='agent', destination_type='tool')
