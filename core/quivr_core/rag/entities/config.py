...
class LLMEndpointConfig(QuivrBaseConfig):
    ...
    def set_api_key(self, force_reset: bool = False):
        ...
        if not self.llm_api_key:
            ...
            # LINEAJE: enforce() `_lineaje_payload` at agent->log log_emit — scan flagged AI_APP_SEC_006 (Use only LLMs from the organization's approved list.); AI_APP_SEC_028 (Do not use LLMs from the organization's disallowed list); AI_IAC_023 (Chatbot and AI interfaces must disclose AI identity to the user). Mask/block; do not remove without review. site_id='site:sha256:6b93fde637d531cb05b24ec1a399edcda9d00da44d82d2e8278fff44e82bb838'
            _gr_client = _lineaje_load_gr_client()
