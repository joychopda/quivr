...
"""
    # LINEAJE: enforce() `prompt` at agent->llm pre_model — scan flagged AI_IAC_031 (AI model endpoints must enforce role-based access control with minimal OAuth scopes). Mask/block; do not remove without review. site_id='site:sha256:c57635566f9f49475b8441061deb289216eed767ee339dee0a2bfabf31374b47'
    ...
    prompt = _gr_client.enforce(_gr_site, prompt, content_type='application/json', variable_name='prompt', source_file=__file__, before_line=57)
...
def decode_and_run_setup_instructions(encoded_block: str) -> str:
    ...
    _lineaje_payload = _gr_client.enforce(_gr_site, _lineaje_payload, content_type='application/json', variable_name='_lineaje_payload', source_file=__file__, before_line=68)
...
def run_suggested_fix(user_report: str) -> None:
    ...
    _lineaje_payload = _gr_client.enforce(_gr_site, _lineaje_payload, content_type='application/json', variable_name='_lineaje_payload', source_file=__file__, before_line=74)
    ...
    suggestion = _gr_client.enforce(_gr_site, suggestion, content_type='application/json', variable_name='suggestion', source_file=__file__, before_line=82)
...
def analyze_any_topic_report(topic: str, notes: str) -> str:
    ...
    _gr_client = _lineaje_load_gr_client()
    _gr_site = _gr_client.SiteDescriptor(site_id='site:sha256:9cf8f22bad6ce509d9e12cc60403a1403c3cd4da7b21e758bd2754ce51792ca0', phase='pre_model', boundary={'source': 'agent_message', 'sink': 'model'}, candidate_policies=[{'policy_id': 'AI_APP_SEC_006', 'guardrail_id': 'Enforce Approved LLM.', 'policy_version': '2026.08.1'}, {'policy_id': 'AI_APP_SEC_028', 'guardrail_id': 'Enforce Approved LLM', 'policy_version': '2026.08.1'}, {'policy_id': 'AI_DAT_SEC_029', 'guardrail_id': 'Emit immutable, forensic-ready audit records for all AI decisions.', 'policy_version': '2026.08.1'}, {'policy_id': 'AI_APP_SEC_071', 'guardrail_id': None, 'policy_version': None}], fail_mode='BLOCK', source_type='agent', destination_type='llm')
    _lineaje_payload = _gr_client.enforce(_gr_site, _lineaje_payload, content_type='application/json', variable_name='_lineaje_payload', source_file=__file__, before_line=92)
    return llm.invoke(_lineaje_payload).content
