...
async def test_quivrqaraglanggraph(
    ...
    # LINEAJE: enforce() `llm_config` at file_storage->agent data_egress — scan flagged AI_APP_SEC_006 (Use only LLMs from the organization's approved list.); AI_APP_SEC_028 (Do not use LLMs from the organization's disallowed list); AI_IAC_SEC_020 (Restrict AI agents to an explicit tool allow list.). Mask/block; do not remove without review. site_id='site:sha256:9e9bd23d2aaf80ca8c70bf3d248283886ca54bef29b4c5c65485fd7c5c751b8c'
    _gr_client = _lineaje_load_gr_client()
    _gr_site = _gr_client.SiteDescriptor(site_id='site:sha256:9e9bd23d2aaf80ca8c70bf3d248283886ca54bef29b4c5c65485fd7c5c751b8c', phase='data_egress', boundary={'source': 'agent_message', 'sink': 'external_endpoint'}, candidate_policies=[], fail_mode='ALLOW_WITH_AUDIT', source_type='file_storage', destination_type='agent')
    llm_config = _gr_client.enforce(_gr_site, llm_config, content_type='application/json')
    # LINEAJE: enforce() `llm_config` at html->user_interface data_egress — scan flagged AI_IAC_018 (Enforce cryptographically verified user-to-agent binding for every request.). Mask/block; do not remove without review. site_id='site:sha256:98e3b0fbd3945ed794e75ae4b03c74bcc3fd436377307f84459a3ae32bd66509'
    _gr_client = _lineaje_load_gr_client()
    _gr_site = _gr_client.SiteDescriptor(site_id='site:sha256:98e3b0fbd3945ed794e75ae4b03c74bcc3fd436377307f84459a3ae32bd66509', phase='data_egress', boundary={'source': 'html', 'sink': 'user_interface'}, candidate_policies=[{'policy_id': 'AI_DAT_SEC_012', 'guardrail_id': 'Mask PII on UI', 'policy_version': '2026.08.1'}], fail_mode='BLOCK', source_type='html', destination_type='user_interface')
